...
public class OrderManagerTest {
    ...
    void payOrderTest_success() {
        PagarNoCredito credito = new PagarNoCredito();
        orderManager.payOrder(order, credito);
    ...
    void payOrderTest_error() {
        PagarNoBoleto boleto = new PagarNoBoleto();
        doThrow(new Exception(("unknown payment method"))).when(paymentService).doPayment();
        orderManager.payOrder(order, boleto);
        InOrder orderVerifier = Mockito.inOrder(paymentService, mailNotification, logService);
        orderVerifier.verify(paymentService).doPayment();
